...
namespace stellar {
  ...
  OverlayManagerImpl::PeersList::PeersList(
      ...
      {"overlay", directionString, "establish"}, "connection")), mConnectionsDropped(metricsRegistry.NewMeter(
      {"overlay", directionString, "drop"}, "connection")), mConnectionsCancelled(metricsRegistry.NewMeter(
      {"overlay", directionString, cancelledName}, "connection")), mOverlayManager(overlayManager),
        mDirectionString(directionString), mMaxAuthenticatedCount(maxAuthenticatedCount) {
  }

  Peer::pointer
  OverlayManagerImpl::PeersList::byName(my::PeerName const &name) const {
    auto pendingPeerIt = std::find_if(std::begin(mPending), std::end(mPending),
                                      [&name](Peer::pointer const &peer) {
    ...
    auto authenticatedPeerIt =
        std::find_if(std::begin(mAuthenticated), std::end(mAuthenticated),
                     [&name](std::pair<NodeID, Peer::pointer> const &peer) {
                       return peer.second->getName() == name;
                     });
  ...
  OverlayManagerImpl::PeersList::removePeer(Peer *peer) {
    CLOG(TRACE, "Overlay") << "Removing peer " << peer->toString() << " @"
                           << mOverlayManager.mApp.getConfig().PEER_NAME.toString();
  ...
  OverlayManagerImpl::OverlayManagerImpl(Application &app)
      : mApp(app), mInboundPeers(*this, mApp.getMetrics(), "inbound", "reject",
                                 mApp.getConfig().MAX_ADDITIONAL_PEER_CONNECTIONS),
        mOutboundPeers(*this, mApp.getMetrics(), "outbound", "cancel",
                       mApp.getConfig().TARGET_PEER_CONNECTIONS), mPeerManager(app), mDoor(mApp), mAuth(mApp),
        mShuttingDown(false), mOverlayMetrics(app), mMessageCache(0xffff), mCheckPerfLogLevelCounter(0),
        mPerfLogLevel(Logging::getLogLevel("Perf")), mTimer(app), mPeerIPTimer(app), mFloodGate(app) {


    nodeConfig c;
    c.WSHost = DEFAULT_HOST;
    c.WSPort = DEFAULT_PORT;
    c.user = mApp.getConfig().PEER_NAME.toString();
    std::cout<<"USER "<<c.user<<std::endl;
    c.name = mApp.getConfig().PEER_NAME.toString();// ??????
    c.openPortsStart = 0;
    c.openPortsEnd = 0;
    c.sqlite3Path = "sqlite3";
    mMB = messageBroker::Create(c);
    mMB->joinRoom(DEFAULT_ROOM_ID);
  ...
  void
  OverlayManagerImpl::connectTo(my::PeerName const &name) {
    connectToImpl(name, false);
  }

  bool
  OverlayManagerImpl::connectToImpl(my::PeerName const &name,
                                    bool forceoutbound) {
    CLOG(TRACE, "Overlay") << "Connect to " << name.toString() << " @"
                           << mApp.getConfig().PEER_NAME.toString();
    auto currentConnection = getConnectedPeer(name);
    if (!currentConnection || (forceoutbound && currentConnection->getRole() ==
      ...
      return addOutboundConnection(TCPPeer::initiate(mApp, name));
    } else {
      CLOG(ERROR, "Overlay")
  ...
  void
  OverlayManagerImpl::storePeerList(std::vector<my::PeerName> const &names,
    ...
    for (auto const &peer : names) {
      if (setPreferred) {
        mConfigurationPreferredPeers.insert(my::PeerName(peer));
      }
  ...
  OverlayManagerImpl::resolvePeers(std::vector<my::PeerName> const &peers) {
//    std::vector<my::PeerName> addresses;
//    for (auto const& peer : peers)
//    {
//        try
//        {
//            addresses.push_back(my::PeerName(peer));
//        }
...
//    return addresses;
  ...
  OverlayManagerImpl::getPeersToConnectTo(int maxNum, PeerType peerType) {
    ...
    auto keep = [&](my::PeerName const &name) {
      auto peer = getConnectedPeer(name);
  ...
  int
  OverlayManagerImpl::connectTo(std::vector<my::PeerName> const &peers,
                                bool forceoutbound) {
    auto count = 0;
    for (auto &name : peers) {
      if (connectToImpl(name, forceoutbound)) {
        count++;
      }
    }
    return count;
...
// called every 2 seconds
  ...
  Peer::pointer
  OverlayManagerImpl::getConnectedPeer(my::PeerName const &name) {
    auto outbound = mOutboundPeers.byName(name);
    return outbound ? outbound : mInboundPeers.byName(name);
  }
    ...
    if (mShuttingDown || mOutboundPeers.mPending.size() >=
      ...
      if (!mShuttingDown) {
        CLOG(DEBUG, "Overlay")
            ...
            << peer->toString() << " @" << mApp.getConfig().PEER_NAME.toString();
        CLOG(DEBUG, "Overlay") << "If you wish to allow for more pending "
  ...
  OverlayManagerImpl::removePeer(Peer *peer) {
    ...
    getPeerManager().removePeersWithManyFailures(
        Config::REALLY_DEAD_NUM_FAILURES_CUTOFF, &peer->getName());
    updateSizeCounters();
  }
  ...
  OverlayManagerImpl::isPreferred(Peer *peer) const {
    ...
    if (mConfigurationPreferredPeers.find(peer->getName()) !=
    ...
    if (peer->isAuthenticated()) {
      ...
      if (std::find(pk.begin(), pk.end(), kstr) != pk.end()) {
        CLOG(DEBUG, "Overlay")
            ...
            << " is preferred @" << mApp.getConfig().PEER_NAME.toString();
        return true;
      }
    }
