...
namespace stellar {
  ...
  Simulation::Simulation(Mode mode, Hash const &networkID, ConfigGen confGen,
      ...
      : mVirtualClockMode(mode != OVER_TCP), mClock(mVirtualClockMode ? VirtualClock::VIRTUAL_TIME
                                                                      : VirtualClock::REAL_TIME), mMode(mode),
        mConfigCount(0), mConfigGen(confGen), mQuorumSetAdjuster(qSetAdjust) {
    ...
    if (mMode == OVER_LOOPBACK)
      ...
      if (iApp) {
        ...
        auto peer = iApp->getOverlayManager().getConnectedPeer(
            my::PeerName{cAcceptor.PEER_NAME});
        if (peer) {
...
//    }
    auto address = my::PeerName{to->getConfig().PEER_NAME};
  ...
  Simulation::haveAllExternalized(uint32 num, uint32 maxSpread) {
    ...
    for (auto it = mNodes.begin(); it != mNodes.end(); ++it) {
      ...
      LOG(DEBUG) << app->getConfig().PEER_NAME.toString() << " @ ledger#: " << n;
